...
// Visuals
constexpr std::string_view X_MARK = "\U00002705";			// ✅ visualization for marked `Min_Term`s
constexpr std::string_view UNUSED = "\U000026D4";			// ⛔ visualization for unmarked `Min_Term`s (going to be a `PRIME_IMPL`)
constexpr std::string_view LEGEND_RED = "\U0001F7E5";		// 🟥 legend symbol RED (Redundant/Unresolved)
constexpr std::string_view LEGEND_GRN = "\U0001F7E9";		// 🟩 legend symbol GRN (Essential)
constexpr std::string_view LEGEND_YLW = "\U0001F7E8";		// 🟨 legend symbol YLW (Core Trigger)
...
// QMC modes (numerical values also used at text formatting)
constexpr uint8_t PHASE = 2;								// PHASE mode (spacing for each decimal = 2)
constexpr uint8_t TABLE = 0;								// TABLE mode (spacing for each decimal = 0)
constexpr bool FINAL_TABLE = true;							// FINAL_TABLE or "Immediate Table"


/**
 * @brief This class executes the entire minimization process using the Quine-McCluskey Tabular Method (QMC) and outputs it in a demonstrative
 * manner — exactly as one would do on paper — for the term passed to it as an argument upon invocation.
 * @date September 2025
 */
class QMC
{
...
private:
	/**
	 * @brief Represents a (DNF) Min_Term during the Quine-McCluskey Tabular Method (QMC).
	 */
	struct Min_Term
	{
